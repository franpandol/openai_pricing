...
class CostAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not all(
            key in request.data
            for key in ["endpoint", "company", "requestBody", "responseBody"]
        ):
        ...
        openai_request_body_decoded = base64.b64decode(openai_request_body).decode(
            "utf-8"
        )
        openai_response_body_decoded = base64.b64decode(openai_response_body).decode(
            "utf-8"
        )
        # validate the json
        try:
        ...
        price = Price.objects.get(
            model=model, company=company, size=size, quality=quality, endpoint=endpoint
        )
        price = price.calculate_price(
            request_body=openai_request_body_decoded,
            response_body=openai_response_body_decoded,
        )
